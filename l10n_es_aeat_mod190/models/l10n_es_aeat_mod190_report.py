...
class L10nEsAeatMod190Report(models.Model):
    ...
    def _get_line_mod190_vals(self, rp, key_id, subkey_id):
        ...
        if not codigo_provincia:
            raise exceptions.UserError(
                self.env._("The state is not defined in the partner, %s") % rp.name
            )
        return {
            "partner_id": rp.id,
    ...
    def _get_grouped_data(self, field_number, domain):
        ...
        return {
            (
                partner.id,
                key.id if key else False,
                subkey.id if subkey else False,
            ): credit - debit
            for partner, key, subkey, credit, debit in groups
        }
